...
{
	int error, entryIndex, i, dirCluster; //, i, j, fd = 0;
	int go_on = 1;
	char buffer[BUFSIZE];
	DirEntry* dirEntry;// = malloc(sizeof(DirEntry));		// Need to populate this (a new one)
	TCB* tcb = getTCB();								// For use by CDIR macro
	int curTask = gettid();								// For use by CDIR macro
	dirCluster = CDIR;
	
	// Check if file exists
	if ((error = fmsGetDirEntry(fileName, &dirEntry)) == 0) 	// Returned dir entry stored in dirEntry
		return E_FILE_ALREADY_DEFINED;

	// Get current cluster
	// cDir in tcb points to the start cluster of the cwd (CDIR macro = tcb[curTask].cdir)
	if (error = fmsReadSector(buffer, 19/*C_2_S(dirCluster)*/)) { 
		printf("error calling readSector in fmsDefineFile...\n");
		return error;
	}
	...
	// Go through all entries in current cluster to see if there's a free slot
	printf("searching through all entries of current cluster...\n");
	while (dirCluster != FAT_EOC && go_on) {
		for (entryIndex = 0; entryIndex < ENTRIES_PER_SECTOR; entryIndex++) {
			printf("EntryIndex: %d\n", entryIndex);
			memcpy(dirEntry, &buffer[entryIndex*sizeof(DirEntry)], sizeof(DirEntry));
			//printf("Name of entry: %s\n", dirEntry->name);
			//printf("Ext of entry: %s\n", dirEntry->extension);
			//printf("Start cluster: %d\n", dirEntry->startCluster);
			//if (dirEntry->name[0] == 0x00 || dirEntry->name[0] == 0xf6) {// Found an available entry, use it
			//	printf("found an open slot in current cluster...\n");
			//	go_on = 0;
			//	break;
			//}
		}
		go_on = 0;
		// We went through all entries in curr cluster and nothing's free,
		// so get a new cluster
		if (go_on) {
			dirCluster = getFatEntry(dirCluster, FAT1);
			if (error = fmsReadSector(buffer, C_2_S(dirCluster))) { 
				printf("error calling readSector in fmsDefineFile...\n");
				return error;
			}
			if (dirCluster == FAT_EOC) {
				// get a free cluster
				// append it onto the FAT chain for the folder currently searching through
				//setFatEntry(/*currentCluster*/CDIR, /*newCluster*/);
				//setFatEntry(/*newCluster*/, FAT_EOC
			}
		}
	}

	printf("Defining file %s...\n", fileName);
	memcpy(dirEntry->name, fileName, strlen(fileName));
	dirEntry->attributes = attribute;
	setDirTimeDate(dirEntry);
	dirEntry->startCluster = dirCluster;
	dirEntry->fileSize = 0;
	fmsWriteSector(buffer, C_2_S(dirCluster));		
...
int fmsOpenFile(char* fileName, int rwMode)
{
	...
	fdEntry->fileSize = (rwMode == OPEN_WRITE) ? 0 : dirEntry.fileSize; // If writing to file, 0 size
	fdEntry->pid = curTask;								// curTask
	fdEntry->mode = rwMode;
	fdEntry->flags = 0;
	fdEntry->fileIndex = (rwMode != OPEN_APPEND) ? 0 : dirEntry.fileSize; // If appending file, go to end
	memset(fdEntry->buffer, -2, BUFSIZE);
	
	if (rwMode == OPEN_APPEND) {									// Appending, fill buff with last cluster
...
int fmsWriteFile(int fileDescriptor, char* buffer, int nBytes)
{
	...
	if (fdEntry->name[0] == 0)		// This fdEntry is still free/hasn't been allocated
		return E_FILE_NOT_OPEN;
	if (fdEntry->mode != OPEN_READ)
		return E_ILLEGAL_ACCESS;

		
	// throw error for FILE SPACE FULL
	// throw undefined error

	// Need to wait until fdEntry's buffer is full, filled via information from buffer,
	// before calling fmsWriteSector, which takes whatever is in fdEntry's buffer
	// and puts them onto disk
	memcpy(fdEntry->buffer, buffer, nBytes);
	fdEntry->fileIndex = fdEntry->fileIndex + nBytes;
	fdEntry->fileSize = fdEntry->fileSize + nBytes;
	...
	// If the buffer for this fd is empty (either we're entering
	// a new sector and just cleared the buffer in a previous
	// iteration of this loop by writing it to disk,
	// or it's never been filled), fill it
	/*if (fdEntry->buffer[0] == -2) {
		if (error = fmsReadSector(fdEntry->buffer, C_2_S(fdEntry->currentCluster)))
			return error;
		// So we're at the next cluster next time for next time
	...
	// Get directory entry so it can be updated
	unsigned char name[8];
	unsigned char ext[3];
	char per[] = ".";
	getFDEntryStrings(fdEntry, name, ext);
